...
INSTANTIATE_RPC_CALL(AsyncService, UpdateBackendList, BackendListUpdates,
                     RpcReply);
INSTANTIATE_RPC_CALL(AsyncService, MarkQueryDroppedByDispatcher, DispatchReply,
                     RpcReply);
...
void RpcService::HandleRpcs() {
  ...
  new MarkQueryDroppedByDispatcher_Call(
      &service_, cq_.get(),
      [this](const grpc::ServerContext&, const DispatchReply& req,
             RpcReply* reply) {
        frontend_->MarkQueryDroppedByDispatcher(req, reply);
      });
