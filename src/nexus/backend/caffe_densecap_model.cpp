...
CaffeDenseCapModel::CaffeDenseCapModel(int gpu_id,
  ...
  fs::path feature_prototxt =
      model_dir / model_info_["feature_prototxt"].as<std::string>();
  fs::path rnn_prototxt =
      model_dir / model_info_["rnn_prototxt"].as<std::string>();
  fs::path embed_prototxt =
      model_dir / model_info_["embed_prototxt"].as<std::string>();
  fs::path model_file = model_dir / model_info_["model_file"].as<std::string>();
  feature_net_.reset(
      new caffe::ServeNet<float>(feature_prototxt.string(), max_batch_));
  rnn_net_.reset(new caffe::ServeNet<float>(rnn_prototxt.string(),
                                            max_batch_ * max_boxes_, 1));
  embed_net_.reset(new caffe::ServeNet<float>(embed_prototxt.string(),
                                              max_batch_ * max_boxes_, 1));
...
Shape CaffeDenseCapModel::InputShape() { return input_shape_; }
...
CaffeDenseCapModel::GetOutputGpuArrays() {
...
void CaffeDenseCapModel::Forward(std::shared_ptr<BatchTask> batch_task) {
  ...
  global_features_blob->Reshape({global_features_blob->shape(0),
                                 total_proposals,
                                 global_features_blob->shape(2)});
  int offset = 0;
  // broadcase global features to same dimension as region features
  for (int i = 0; i < batch; ++i) {
    caffe::caffe_gpu_gemm(CblasNoTrans, CblasNoTrans, num_proposals[i], nfeat,
                          1, (float)1., multiplier_->gpu_data(),
                          _global_features_blob->gpu_data(), (float)0.,
                          global_features_blob->mutable_gpu_data() + offset);
...
void CaffeDenseCapModel::Postprocess(std::shared_ptr<Task> task) {
  ...
  for (int i = 0; i < nboxes; ++i) {
    ...
    for (int j = 0; j < 4; ++j) {
      bbox_offsets[i * 4 + j] =
          bbox_offsets[i * 4 + j] * bbox_stds_[j] + bbox_mean_[j];
    }
  }
...
void CaffeDenseCapModel::TransformBbox(int im_height, int im_width, float scale,
                                       int nboxes, const float* rois,
  ...
  for (int i = 0; i < nboxes; ++i) {
    ...
    out[i * 4 + 1] =
        std::max(std::min(ctr_y - 0.5 * height, im_height - 1.), 0.);
