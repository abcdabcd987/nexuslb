...
Shape SharePrefixModel::InputShape() { return prefix_model_->InputShape(); }

std::unordered_map<std::string, Shape> SharePrefixModel::OutputShapes() {
  std::lock_guard<std::mutex> lock(suffix_mu_);
  return {{"output", Shape({static_cast<int>(max_batch_),
                            static_cast<int>(max_suffix_output_size_)})}};
}
...
SharePrefixModel::GetOutputGpuArrays() {
...
void SharePrefixModel::Forward(std::shared_ptr<BatchTask> batch_task) {
  ...
  auto tasks = batch_task->tasks();
  float* prefix_batch_output_ptr =
      prefix_batch_output_arr_.at(prefix_output_name_)->Data<float>();
  size_t prefix_output_nfloats = prefix_output_shape_.NumElements(1);
  size_t cpu_output_offset = 0;
  std::vector<std::tuple<std::shared_ptr<ModelInstance>,
                         std::shared_ptr<BatchTask>, ArrayPtr> >
      suffix_batches;
  ...
  for (int i = 0; i < tasks.size();) {
    ...
    auto suffix_batch_input_arr =
        suffix_model->CreateInputGpuArrayWithRawPointer(
    ...
    for (int j = 0; j < suffix_tasks.size(); ++j) {
      ...
      task->suffix_model = suffix_model;
      auto suffix_input_arr =
          suffix_batch_input_arr->Slice(input_offset, prefix_output_nfloats);
...
bool SharePrefixModel::AddModelSession(const ModelSession& model_sess) {
  ...
  for (auto& iter : suffix_models_) {
    ...
    pref_len = ModelDatabase::Singleton().GetSharePrefixLength(model_id,
                                                               base_model_id);
    break;
  }
  if (pref_len != prefix_length_) {
    LOG(ERROR) << "New prefix length is not same as old (" << pref_len << " vs "
               << prefix_length_ << ")";
    return false;
  }
