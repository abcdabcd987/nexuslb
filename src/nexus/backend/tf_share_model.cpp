...
TFShareModel::TFShareModel(int gpu_id, const ModelInstanceConfig& config)
    : ModelInstance(gpu_id, config), num_suffixes_(0) {
  for (int i = 0; i < config.model_session_size(); ++i) {
    ...
    if (!tf_share_info_) tf_share_info_ = info;
    CHECK(tf_share_info_ == info)
        << "Cannot run different TFShare models in a single instance. ("
        << tf_share_info_->hack_internal_id << ") vs ("
        << info->hack_internal_id << ").";
  }
  ...
  for (const auto& iter : tf_share_info_->suffix_models) {
    if (iter.second.class_names.empty()) continue;
...
Shape TFShareModel::InputShape() { return tf_model_->InputShape(); }
...
void TFShareModel::Forward(std::shared_ptr<BatchTask> batch_task) {
  ...
  for (size_t i = 0; i < tasks.size();) {
    ...
    CHECK(iter != tf_share_info_->suffix_models.end())
        << "Cannot find model " << model_sess_id << " in "
        << tf_share_info_->hack_internal_id;
    const auto suffix_index = iter->second.suffix_index;
    CHECK_EQ(slice_len[suffix_index], 0)
        << "Detected non-consecutive BatchTask";
  ...
  auto in_tensor = m.input_tensors_[batch_task->GetInputArray()->tag()]->Slice(
      0, batch_size);
  ...
  tf::Status status =
      m.session_->Run(inputs, m.output_layers_, {}, &out_tensors);
  if (!status.ok()) {
  ...
  for (size_t i = 0; i < m.output_layers_.size(); ++i) {
    ...
    for (int32_t j = 0; j < len; ++j) {
      ...
      const char* tensor_data =
          tensor_data_base + j * num_elements * sizeof(float);
      Memcpy(out_buf->Data<float>(), cpu_device_, tensor_data, cpu_device_,
             num_elements * sizeof(float));
      arr[name] = out_buf;
      const auto& input = batch_task->inputs()[beg + j];
      outputs[beg + j] =
          std::make_shared<Output>(input->task_id, input->index, arr);
    }
  }
...
void TFShareModel::Postprocess(std::shared_ptr<Task> task) {
  ...
  auto suffix_info_iter =
      tf_share_info_->suffix_models.find(model_sess.model_name());
