...
void RoundRobinScheduler::WorkFullSchedule() {
...
RoundRobinScheduler::DropTimeoutQueries() {
  auto now = Clock::now();
...
std::optional<BatchPlan> RoundRobinScheduler::TryScheduleModelSessionOnBackend(
...
void RoundRobinScheduler::WorkFinalizePlan(NodeId backend_id, PlanId plan_id) {
  using namespace std::chrono;
