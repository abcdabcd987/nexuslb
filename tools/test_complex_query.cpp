...
DEFINE_int32(
    avg_interval, 10,
    "Moving average interval for backend rate");  // for the sch_info.cpp
                                                  // linking error

ComplexQuery::NodeID add_node(ComplexQuery &cq, const std::string &gpu,
                              const std::string &framework,
                              const std::string &model_name, int image_width,
                              int image_height) {
  ...
  auto *profile =
      ModelDatabase::Singleton().GetModelProfile(gpu, "generic", profile_id);
  CHECK(profile != nullptr);
  cq.AddNode(node, model_sess_id, *profile);
  return node;
}

void add_node(ComplexQuery &cq, ComplexQuery::NodeID &node,
              const std::string &profile_id, const std::string &gpu) {
...
int main(int argc, char **argv) {
  ...
  for (auto &node : split)
    std::cout << "  " << node.first.ToString() << ": " << node.second << "ms"
              << std::endl;
}
